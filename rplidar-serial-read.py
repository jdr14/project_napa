...
################################
    def startScan(self, port):
        ...
        while lock == False:
            ...
            #frame of A55A
            port.write(Start_Scan.encode())
            try:
                #If after looping nothing found,
                #Reset and try again
                for a in range(0, 250):
                    character = port.read()
                    line += character.decode('Latin-1')
                    if (line[0:2] == "\xa5\x5a"):
                        if(len(line) == 7):
                            lock = True
                            break
...
################################
    ...
    def getPoints(self,port,polar=True):
        line = ""
        if polar:
            print("Printing coordinates in polar")
        while True:
            try:
                character = port.read()
                line += character.decode('Latin-1')
                ...
                if (len(line) == 5):
                    ...
                    if polar == True:
                        distance, angle = self.point_Polar(line)
                        if (angle > 260 and angle < 280 and distance < 1000): # Note 180 is behind the lidar sensor
                            print(f"angle: {angle:.2f} | distnace: {distance:.2f}")
...
################################
    def point_Polar(self,serial_frame,radians=False):
        #Get Distance
        #serial_frame[3].encode("hex")
        distance = codecs.encode(serial_frame[4].encode(),"hex") + codecs.encode(serial_frame[3].encode(),"hex")
        ...
        angle = codecs.encode(serial_frame[2].encode(),"hex") + codecs.encode(serial_frame[1].encode(),"hex")
